...
-- membuang struktur untuk table dss_analytics.cache
...
-- Membuang data untuk tabel dss_analytics.cache: ~0 rows (lebih kurang)

-- membuang struktur untuk table dss_analytics.cache_locks
...
-- Membuang data untuk tabel dss_analytics.cache_locks: ~0 rows (lebih kurang)

-- membuang struktur untuk table dss_analytics.charts
CREATE TABLE IF NOT EXISTS `charts` (
  ...
  `embed_token` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `cache_ttl_seconds` int NOT NULL DEFAULT '300',
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `charts_embed_token_unique` (`embed_token`),
  KEY `charts_creator_id_foreign` (`creator_id`),
  CONSTRAINT `charts_creator_id_foreign` FOREIGN KEY (`creator_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Membuang data untuk tabel dss_analytics.charts: ~3 rows (lebih kurang)
INSERT INTO `charts` (`id`, `title`, `description`, `raw_query`, `chart_type`, `config`, `creator_id`, `embed_token`, `cache_ttl_seconds`, `created_at`, `updated_at`) VALUES
	(2, 'Grafik Jumlah User per Role', 'total anggota tiap role nya', 'SELECT roles.name as role_name, COUNT(users.id) as total \nFROM users \nJOIN roles ON users.role_id = roles.id \nGROUP BY roles.name', 'pie', '{"x_axis": "role_name", "y_axis": "total"}', 1, 'Pr87yHFr9uzT0c0VHCbPPFz8AcwCpdW81qFwBqN9', 300, '2026-08-17 19:16:17', '2026-08-17 20:50:55'),
	(4, 'Pendapatan Berdasarkan Kategori Produk', NULL, 'SELECT \n    p.productLine AS kategori_produk, \n    SUM(od.quantityOrdered * od.priceEach) AS total_pendapatan\nFROM orderdetails od\nJOIN products p ON od.productCode = p.productCode\nJOIN orders o ON od.orderNumber = o.orderNumber\nWHERE o.status = \'Shipped\'\nGROUP BY p.productLine\nORDER BY total_pendapatan DESC;', 'pie', '{"x_axis": "kategori_produk", "y_axis": "total_pendapatan"}', 1, 'QR9rfK972gQBodJa8PEGphYjjFcQk0mx3yEjkfJB', 300, '2026-08-18 01:28:17', '2026-08-18 01:29:37'),
	(5, 'Tren Penjualan Bulanan', NULL, 'SELECT \n    MONTHNAME(paymentDate) AS bulan,\n    SUM(amount) AS total_pemasukan\nFROM payments\nWHERE YEAR(paymentDate) = 2004\nGROUP BY MONTH(paymentDate), MONTHNAME(paymentDate)\nORDER BY MONTH(paymentDate);', 'line', '{"x_axis": "bulan", "y_axis": "total_pemasukan"}', 1, '2OS61sajAWzrVb41gYhObbvARObG5hJis1DGDpv1', 300, '2026-08-18 01:31:42', '2026-08-18 01:34:07');

-- membuang struktur untuk table dss_analytics.chart_role
CREATE TABLE IF NOT EXISTS `chart_role` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  ...
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `chart_role_chart_id_foreign` (`chart_id`),
  KEY `chart_role_role_id_foreign` (`role_id`),
  CONSTRAINT `chart_role_chart_id_foreign` FOREIGN KEY (`chart_id`) REFERENCES `charts` (`id`) ON DELETE CASCADE,
  CONSTRAINT `chart_role_role_id_foreign` FOREIGN KEY (`role_id`) REFERENCES `roles` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=12 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Membuang data untuk tabel dss_analytics.chart_role: ~6 rows (lebih kurang)
INSERT INTO `chart_role` (`id`, `chart_id`, `role_id`, `created_at`, `updated_at`) VALUES
	(5, 2, 1, NULL, NULL),
	(6, 2, 2, NULL, NULL),
	(7, 2, 3, NULL, NULL),
	(8, 2, 4, NULL, NULL),
...
-- membuang struktur untuk table dss_analytics.customers
...
-- Membuang data untuk tabel dss_analytics.customers: ~122 rows (lebih kurang)
...
-- membuang struktur untuk table dss_analytics.employees
...
-- Membuang data untuk tabel dss_analytics.employees: ~23 rows (lebih kurang)
...
-- membuang struktur untuk table dss_analytics.failed_jobs
...
-- Membuang data untuk tabel dss_analytics.failed_jobs: ~0 rows (lebih kurang)

-- membuang struktur untuk table dss_analytics.jobs
...
-- Membuang data untuk tabel dss_analytics.jobs: ~0 rows (lebih kurang)

-- membuang struktur untuk table dss_analytics.job_batches
...
-- Membuang data untuk tabel dss_analytics.job_batches: ~0 rows (lebih kurang)

-- membuang struktur untuk table dss_analytics.migrations
...
-- Membuang data untuk tabel dss_analytics.migrations: ~0 rows (lebih kurang)
INSERT INTO `migrations` (`id`, `migration`, `batch`) VALUES
	(1, '0000_01_01_000000_create_roles_table', 1),
	(2, '0001_01_01_000000_create_users_table', 1),
	(3, '0001_01_01_000001_create_cache_table', 1),
	(4, '0001_01_01_000002_create_jobs_table', 1),
	(5, '2026_08_13_082802_create_charts_table', 1),
	(6, '2026_08_13_082803_create_chart_role_table', 1),
	(7, '2026_08_13_084929_create_personal_access_tokens_table', 2);

-- membuang struktur untuk table dss_analytics.offices
...
-- Membuang data untuk tabel dss_analytics.offices: ~7 rows (lebih kurang)
...
-- membuang struktur untuk table dss_analytics.orderdetails
...
-- Membuang data untuk tabel dss_analytics.orderdetails: ~2.996 rows (lebih kurang)
...
-- membuang struktur untuk table dss_analytics.orders
...
-- Membuang data untuk tabel dss_analytics.orders: ~326 rows (lebih kurang)
...
-- membuang struktur untuk table dss_analytics.password_reset_tokens
...
-- Membuang data untuk tabel dss_analytics.password_reset_tokens: ~0 rows (lebih kurang)

-- membuang struktur untuk table dss_analytics.payments
...
-- Membuang data untuk tabel dss_analytics.payments: ~273 rows (lebih kurang)
...
-- membuang struktur untuk table dss_analytics.personal_access_tokens
...
-- Membuang data untuk tabel dss_analytics.personal_access_tokens: ~18 rows (lebih kurang)
INSERT INTO `personal_access_tokens` (`id`, `tokenable_type`, `tokenable_id`, `name`, `token`, `abilities`, `last_used_at`, `expires_at`, `created_at`, `updated_at`) VALUES
	(4, 'App\\Models\\User', 1, 'auth_token', 'dc294c64ce274e5f1d6b77918c336485db3facdfe01862e8800909f1ce04ef83', '["*"]', '2026-08-13 18:52:11', NULL, '2026-08-13 18:52:07', '2026-08-13 18:52:11'),
	(8, 'App\\Models\\User', 1, 'auth_token', 'e8b960b4cce3afa243c5901fe4bc191de71d8745e0a23cc835ac5f42b6b3c4f5', '["*"]', '2026-08-13 19:30:09', NULL, '2026-08-13 19:30:06', '2026-08-13 19:30:09'),
	(11, 'App\\Models\\User', 1, 'auth_token', '64c6ba909991f1935bb280bfc39f4a2c1f874fbf44e7f6f5075504f48e8c8ea1', '["*"]', NULL, NULL, '2026-08-13 23:15:18', '2026-08-13 23:15:18'),
	(12, 'App\\Models\\User', 1, 'auth_token', '27926f112b726a7ac6dddbc547c6bad960f1e9dd92389b8a065e49b9168d0d8e', '["*"]', NULL, NULL, '2026-08-13 23:15:20', '2026-08-13 23:15:20'),
	(61, 'App\\Models\\User', 1, 'auth_token', '04338939832f9bdac8fdd4fbbf9925093c7feec1508e0f021b4b3eea13e5a088', '["*"]', NULL, NULL, '2026-09-02 18:21:00', '2026-09-02 18:21:00'),
	(62, 'App\\Models\\User', 1, 'auth_token', '1d7d294aca16bc567d6bc823f62747e3825eeac631becf32405d12deb944888e', '["*"]', '2026-09-02 18:21:45', NULL, '2026-09-02 18:21:19', '2026-09-02 18:21:45'),
	(63, 'App\\Models\\User', 1, 'auth_token', 'c2a3491e133942e892b862ba035ed81f0703ced1ab76f2e43f0e249598ae3111', '["*"]', '2026-09-02 18:23:14', NULL, '2026-09-02 18:21:41', '2026-09-02 18:23:14');

-- membuang struktur untuk table dss_analytics.productlines
...
-- Membuang data untuk tabel dss_analytics.productlines: ~7 rows (lebih kurang)
...
-- membuang struktur untuk table dss_analytics.products
...
-- Membuang data untuk tabel dss_analytics.products: ~110 rows (lebih kurang)
...
-- membuang struktur untuk table dss_analytics.roles
...
-- Membuang data untuk tabel dss_analytics.roles: ~4 rows (lebih kurang)
INSERT INTO `roles` (`id`, `name`, `created_at`, `updated_at`) VALUES
	(1, 'Data Analyst', '2026-08-13 01:32:51', '2026-08-13 01:32:51'),
	(2, 'CEO', '2026-08-13 01:32:51', '2026-08-13 01:32:51'),
	(3, 'Manager HR', '2026-08-13 01:32:51', '2026-08-13 01:32:51'),
	(4, 'Manager IT', '2026-08-13 01:32:51', '2026-08-13 01:32:51'),
	(6, 'Manajemen Keuangan', '2026-08-18 01:21:21', '2026-08-18 01:21:21');

-- membuang struktur untuk table dss_analytics.sessions
...
-- Membuang data untuk tabel dss_analytics.sessions: ~0 rows (lebih kurang)

-- membuang struktur untuk table dss_analytics.users
...
-- Membuang data untuk tabel dss_analytics.users: ~6 rows (lebih kurang)
INSERT INTO `users` (`id`, `name`, `email`, `email_verified_at`, `password`, `role_id`, `remember_token`, `created_at`, `updated_at`) VALUES
	(1, 'Moh. Syaeful Effendi', 'mohsyaefuleffendi@student.uns.ac.id', NULL, '$2y$12$8jesM9cS4hjCZB8H7ikbHuvs3LugOM040wfYZbx3NCmOhkyxNR0L2', 1, NULL, '2026-08-13 01:32:52', '2026-09-02 00:42:21'),
	(2, 'Firstasya Cantika Nava Herlianty', 'firstasya@gmail.com', NULL, '$2y$12$2vUisNhee24plPxMbT8inet06W4zYNSr8oqGAxgcS0g/IIWJWwPtG', 2, NULL, '2026-08-17 20:14:43', '2026-08-17 20:59:40'),
	(3, 'Ramadhani Sesionanda Ispriadi', 'ramadhani@gmail.com', NULL, '$2y$12$ort7E4unDFCQtGMFZteTbula0/gd2fPS9jLWmedh4DmlQCaIGw/EK', 3, NULL, '2026-08-17 20:27:00', '2026-08-17 20:27:00'),
	(4, 'Cahya Dwi Saputra', 'cahyadsap@gmail.com', NULL, '$2y$12$5Bg/fpQLDkrnvvUwElhvc.BX12/dxr7RKdLG84qwYfDn3wxNlJyh6', 4, NULL, '2026-08-17 20:28:04', '2026-08-17 20:28:04'),
	(5, 'Afrida Ardhyatians Herliana', 'liana@gmail.com', NULL, '$2y$12$nFTUlBuFR88jXv0Iio8EDuOWC18swQrYVl415scaQj2E8zXQ9i98G', 2, NULL, '2026-08-17 21:45:12', '2026-08-17 23:08:59'),
	(7, 'Rahma', 'rahma@gmail.com', NULL, '$2y$12$jz5Ctpt2Tab6xUoIZCZvFuckOAQGHgGlPblR4XHrBscI8tle2YDPO', 6, NULL, '2026-08-18 01:27:00', '2026-08-18 01:27:00');
